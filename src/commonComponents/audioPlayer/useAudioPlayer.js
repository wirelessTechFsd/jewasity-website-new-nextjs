...
export const useAudioPlayer = (modalShow, videosList) => {
  ...
  const calculateVideoHeight = (width, height) => {
    ...
    if (height < 590) return 400;
    if (height < 690) return 400;
    if (height < 768) return 400;
    if (height < 810) return 800;
    if (height < 992) return 800;
    if (height < 1050) return 800;
    return 800;
  };
  const videoHeight = calculateVideoHeight(width, height);
  ...
  const handleDownload = () => {
    const videoName = playerRef?.current?.props?.url?.split("com/")[1];
